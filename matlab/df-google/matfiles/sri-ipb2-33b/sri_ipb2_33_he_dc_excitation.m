...
experiment_name = 'sri-ipb2-33-he-dc-excitation.csv';
matfile = strcat(containing_dir, filesep, ...
  strrep(experiment_name, '.csv', '.mat'));
parameter_file = 'sri-ipb2-33-he-dc-excitation.mat';
calibration_file = 'sri-ipb2-33b-he-dc-calibration_DKF_calibration_01.mat';
...
% run_data.model.type = 'two state model with two a-node inputs using T modelled and LPF';
run_data.fit.max_iter = 10;

%% Run calibration

if true % predict response from a calibration run's parameters
  if exist(calibration_file, 'file') == 2 % ensure calibration file exists
    S = load(calibration_file);
    if strcmp(run_data.model.type, S.run_data.model.type) % ensure models match
