...
qpulse = 1;
%input reactor
reactor = 'ipb2-sep09-166'  %'ipb1','ipb2-aug','ipb2-sep05' 'ipb2-sep07',ipb2-sep09-165
switch (reactor)
case 'ipb2-sep09-166'
    ...
    for ki = 1:1:5  
      ...
      for ni = nj1:1:nj2-1
        if (j2(ni+1,10)-j2(ni,10))==1 %noQ
          j3 = j2(nj1:ni,:);
          fn = ['C:\jinwork\BEC\tmp\' reactor num2str(whichSeq) num2str(temp) num2str(qL(ki)) num2str(ki) 'nq.csv'];          
...
%j1 = horzcat(dateN,HeaterPower,CoreTemp,QPulseLengthns,QkHz,QPow,TerminationHeatsinkPower,PressureSensorPSI,QPulsePCBHeatsinkPower,QEnable);
        if ((j2(ni,4)==qL(ki) & j2(ni+1,4)==qL(ki+1) & j2(ni,10) == 1) || (j2(ni,10)-j2(ni+1,10))==1 ) 
           ...
           %qPCB1(ki) = trimmean(j2(nj1:ni,9),25);
           %qTerm1(ki) = trimmean(j2(nj1:ni,7),25);
           %qPow1(ki) = trimmean(j2(nj1:ni,6),25);
           dt1(ki) = j2(ni,1);
           j5(ki) = ni-nj1;
           nj1 = ni+1;     
           break
    ...
    tpi = [temp heatPower0 heatPower qPCB qTerm qPow coreQPow qPCB1 qTerm1 qPow1 j5];
    tp = vertcat(tp,tpi);
    dti=[dt0 dt1];
    dt2 = vertcat(dt2,dti); 
  end    
