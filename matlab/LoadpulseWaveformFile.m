...
for wi = 1:5
   ...
   zterm = table2array(waveform(wi,7));
   filterValue = table2array(waveform(wi,8));
   M = csvread(fn,nh,0);
   max2 = max(M(:,2));
   min2 = min(M(:,2));
   y1 = [0,max2(1)];
   y = [min2(1),max2(1)];
   maxP = find(M(:,2) > factor*max2);
   minP = find(M(:,2) < factor*min2);
   fstMax = maxP(1);
   fstMin = minP(1);
