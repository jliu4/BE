...
switch (whichEx)
    case 1 
        ...
        rowPerFigure = 1;
        columnPerFigure = 1;
        SYS = [SYS 'H2-150C-400C-DEBUG-NEW-SW'];
    case 2
      ...
      for ki = 1:1:5  
         ...
         for ni = nj1:1:nj2-1
             if (j2(ni+1,10)-j2(ni,10))==1 %noQ
                j3 = j2(nj1:ni,:);
                fn = ['C:\jinwork\BEC\tmp\ipb2-' num2str(whichEx) num2str(temp) num2str(qw(ki)) num2str(ki) '.csv'];          
         ...
         %TODO bugs 
             if (j2(ni,7)==qw(ki) & j2(ni+1,7)==qw(ki+1) & j2(ni,10) == 1)
...
fn = ['C:\jinwork\BEC\tmp\ipb2-' num2str(whichEx) '.xlsx'];            
