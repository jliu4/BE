...
while (i < dataSize-1)  
  ...
  if (abs(data.SeqStepNum(i+1) - data.SeqStepNum(i)) >= 1  || i == dataSize-1 ) %sequence changed or last sequence
    ...
    if hpExpFit
      ...
      hpFit(seq2,1:5) = expFit(data.HeaterPower(i1+i11:i2),figname,hp(seq2),coreT(seq2),tStr,seq2,ff,pos,0);      
...
if (writeOutput)
    if tempExpFit
T1=[T1;table(coreT(:),inT(:),outT(:),ql(:),qf(:),hp(:),coreQPow(:),v1(:),v2(:),v3(:),qPow(:),qSP(:),qSV(:),qCur(:),termP(:),pcbP(:),...
