function plotFFT(M,t1,t2,pos,figname,tt,Fs,xrange,visible )
    ...
    % fre = numPoint/nfft
     ...
     subplot(2,2,2)
     %yyaxis left
     plot(F(1:nfft/2)/1e3,20*log10(mY1(1:nfft/2)),F(1:nfft/2)/1e3,20*log10(mY2(1:nfft/2)),F(1:nfft/2)/1e3,20*log10(mY3(1:nfft/2)));
    ...
    % yyaxis right
     %cla
     %cla reset
     %plot(F(1:nfft/2)/1e3,pY1(1:nfft/2),F(1:nfft/2)/1e3,pY2(1:nfft/2),F(1:nfft/2)/1e3,pY3(1:nfft/2));
     %grid on;
     %grid minor;
     %ylabel('radians')
    ...
    [P2,F]=periodogram(M(t1:t2,3),[],nfft,Fs,'power');
     ...
     plot(F,10*log10(P1),F,10*log10(P2),F,10*log10(P3));
     %legend('v1','v2','v3');
